...
use num::{BigInt, Rational, Rational64, BigRational};
    ...
    for cycle in cycles {
        if tzstats_exclude.contains(&cycle) {
            ...
            for (idx, delegate) in delegates.iter().enumerate() {
                ...
                compare_rewards(tzstats_response, tezedge_delegate_split);
            }
        }
...
pub type TezedgeRewardsResponse = BTreeMap<String, TezedgeDelegatorRewards>; 
pub type TezedgeDelegatorRewardsInt = BTreeMap<String, BigInt>;
pub type TezedgeDelegatorRewards = BTreeMap<String, String>;
...
fn compare_rewards<T: IndexerReward>(indexer_rewards: T, tezedge_delegator_rewards: &TezedgeDelegatorRewards) {
    let indexer_delegate_reward = indexer_rewards.total_rewards();
    let indexer_staking_balance = indexer_rewards.staking_balance();

    let tezedge_delegator_rewards_int: TezedgeDelegatorRewardsInt = tezedge_delegator_rewards.iter().map(|(delegator, reward)| {
        (delegator.clone(), BigInt::from_str(reward).unwrap())
    })
    .collect();

    let indexer_delegator_rewards: BTreeMap<String, BigInt> = indexer_rewards.delegators().iter().map(|delegator| {
        (delegator.address.clone(), calculate_reward_share(indexer_staking_balance, indexer_delegate_reward, delegator.balance))
    })
    .collect();
    ...
    if indexer_delegator_rewards.len() != tezedge_delegator_rewards_int.len() {
        println!("Delegator count difference - tezedge({}) indexer({})", tezedge_delegator_rewards_int.len(), indexer_delegator_rewards.len())
    } else {
        assert_eq!(tezedge_delegator_rewards_int, indexer_delegator_rewards);
    }
...
fn calculate_reward_share(staking_balance: i64, total_rewards: i64, delegator_balance: i64) -> BigInt {
...
async fn get_tzkt_rewards_split(cycle: i32, delegate: &str) -> Result<TzktRewardSplitResponse, anyhow::Error> {
    ...
    let url_as_string = format!("{}/{}/{}?limit=10000", TZKT_API_SPLIT_ENDPOINT, delegate, cycle);
...
async fn get_tezedge_rewards(cycle: i32) -> Result<TezedgeRewardsResponse, anyhow::Error> {
    // https://api.ithacanet.tzkt.io/v1/rewards/split/tz1RuHDSj9P7mNNhfKxsyLGRDahTX5QD1DdP/124
    const TEZEDGE_REWARDS_RPC_ENDOPOINT: &str = "http://116.202.128.230:18732/dev/rewards/cycle/";
...
// TODO: duplicate body...
async fn get_tzstats_rewards(cycle: i32, delegate: &str) -> Result<TzstatsRewardsResponse, anyhow::Error> {
    // https://api.ithacanet.tzkt.io/v1/rewards/split/tz1RuHDSj9P7mNNhfKxsyLGRDahTX5QD1DdP/124
    const TZSTATS_REWARDS_RPC_ENDOPOINT: &str = "https://api.ithaca.tzstats.com/explorer/bakers";

    let url_as_string = format!("{}/{}/snapshot/{}", TZSTATS_REWARDS_RPC_ENDOPOINT, delegate, cycle);
    let url = url_as_string
        .parse()
        .unwrap_or_else(|_| panic!("Invalid URL: {}", &url_as_string));
    ...
    if status_code.is_success() {
        ...
        panic!("Request failed with status: {}\nBody: {:#?}", status_code, String::from_utf8_lossy(&dst))
    }
}
